...
Normal paragraph at the end.`);
  ...
  const renderTreeNode = (node, depth = 0) => {
    ...
    return (
      <div
        key={`${depth}-${node.type}-${Math.random()}`}
      ...
      >
        ...
        {node.children &&
          node.children.map((child, index) => renderTreeNode(child, depth + 1))}
      </div>
    );
  };
      ...
      <div className="mt-6">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded p-4">
          ...
          <ul className="text-sm space-y-1">
            <li>
              • <strong>Bold text</strong> should show as <code>strong</code>{" "}
              nodes
            </li>
            <li>
              • <strong>Italic text</strong> should show as{" "}
              <code>emphasis</code> nodes
            </li>
            <li>
              • <strong>Lists</strong> should show as <code>list</code> nodes
              with <code>ordered: true/false</code>
            </li>
            <li>
              • <strong>Headings</strong> should show as <code>heading</code>{" "}
              nodes with <code>depth</code>
            </li>
            <li>
              • <strong>Code</strong> should show as <code>code</code> or{" "}
              <code>inlineCode</code> nodes
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div className="bg-white border rounded p-2">
            ...
            <div
              className={
                parsedTree?.children?.some((child) => child.type === "list")
                  ? "text-green-600"
                  : "text-red-600"
              }
            >
          ...
          <div className="bg-white border rounded p-2">
            ...
            <div
              className={
                parsedTree?.children?.some((child) => child.type === "heading")
                  ? "text-green-600"
                  : "text-red-600"
              }
            >
